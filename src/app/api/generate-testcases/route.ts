...
Do NOT wrap your response in markdown tags. The output must start with { and end with }.
...
`;
    ...
    if (story.source?.toLowerCase() === 'swagger' || story.source?.toLowerCase() === 'openapi') {
...
3. Explicitly test standard UI security/validation checks (like XSS in text inputs, SQL injection in search fields) using concrete injection strings.`;
    ...
    try {
      ...
      console.error("JSON Parse Error. Raw string:", cleanContent?.substring(0, 500));
